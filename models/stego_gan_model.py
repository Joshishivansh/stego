...
class stegoganmodel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()), lr=opt.lrG, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()), lr=opt.lrD, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        self.rec_B = self.netG_A(self.fake_A + gaussian_noise.to(self.device), self.latent_real_B)
